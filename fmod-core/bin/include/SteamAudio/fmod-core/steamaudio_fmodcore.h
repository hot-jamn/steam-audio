...
// Manages assigning a 32-bit integer handle to IPLSource objects, so Unity scripts can reference a specific IPLSource
// in a single call to the FMOD Core API.
class SourceManager
{
public:
    SourceManager();
...
typedef enum IPLFMODCoreSpatializerParams
{
     ...
     *
     *  World-space position of the source. Automatically written by FMOD Core.
     */
     ...
     *  **Type**: `FMOD_DSP_PARAMETER_TYPE_DATA`
     *
     *  Overall linear gain of this effect. Automatically read by FMOD Core.
     */
     ...
     *  -   `2`: Use a distance attenuation value calculated using the curve specified in the FMOD Core UI.
     */
     ...
     *  be obtained by calling `iplFMODCoreAddSource`.
     */
...
/**
 *  DSP parameters for the "Steam Audio FMOD Core Reverb" effect.
 */
...
/**
 *  DSP parameters for the "Steam Audio FMOD Core Mixer Return" effect.
 */
...
// Enhanced mixer return parameter constants for compatibility
#define MIXRETURN_PARAM_BINAURAL IPL_FMODCORE_MIXRETURN_BINAURAL
#define MIXRETURN_PARAM_MIXLEVEL 2
#define MIXRETURN_PARAM_AMBISONICSORDER 3
#define MIXRETURN_PARAM_OUTPUTFORMAT IPL_FMODCORE_MIXRETURN_OUTPUT_FORMAT

// Enhanced reverb parameter constants for compatibility
#define REVERB_PARAM_BINAURAL IPL_FMODCORE_REVERB_BINAURAL
#define REVERB_PARAM_MIXLEVEL 2
#define REVERB_PARAM_OUTPUTFORMAT IPL_FMODCORE_REVERB_OUTPUT_FORMAT

// Speaker format parameter constants
#define PARAMETER_MONO 0
#define PARAMETER_STEREO 1
#define PARAMETER_QUADRAPHONIC 2
#define PARAMETER_SURROUND_5_1 3
#define PARAMETER_SURROUND_7_1 4
#define PARAMETER_NUM_SPEAKER_FORMAT_TYPES 5
...
/**
 *  Initializes the FMOD Core integration. This function must be called before creating any Steam Audio DSP effects.
 *  If a shared context is available from Unity, it will be used; otherwise, a new context will be created.
 *
 *  \param  context The Steam Audio context created by the game engine when initializing Steam Audio. Can be nullptr
 *                  if using shared context from Unity.
 */
F_EXPORT void F_CALL iplFMODCoreInitialize(IPLContext context);

/**
 *  Shuts down the FMOD Core integration. This function must be called after all Steam Audio DSP effects have been
 *  destroyed.
 */
 ...
 *  initialization, after \c iplFMODCoreInitialize. It should also be called whenever the game engine needs to change the
 *  HRTF.
 *
 ...
 *  This function must be called once during initialization, after \c iplFMODCoreInitialize.
 *
...
/**
 *  Adds an IPLSource to the source manager and returns a handle that can be used to reference it.
 *
 *  \param  source  The IPLSource object to add.
 *  \return         A handle that can be used to reference this source.
 */
F_EXPORT IPLint32 F_CALL iplFMODCoreAddSource(IPLSource source);

/**
 *  Removes an IPLSource from the source manager using its handle.
 *
 *  \param  handle  The handle of the source to remove.
 */
F_EXPORT void F_CALL iplFMODCoreRemoveSource(IPLint32 handle);

/**
 *  Enables or disables HRTF processing globally.
 *
 *  \param  disabled  If true, HRTF processing is disabled.
 */
F_EXPORT void F_CALL iplFMODCoreSetHRTFDisabled(bool disabled);

/**
 *  Sets the shared context from Unity. This allows the FMOD Core integration to use the same Steam Audio context
 *  as Unity, enabling seamless integration with Unity's probe placement and baking workflow.
 *
 *  \param  context  The shared Steam Audio context from Unity.
 */
F_EXPORT void F_CALL iplFMODCoreSetSharedContext(IPLContext context);

/**
 *  Gets the current shared context. Returns nullptr if no shared context is available.
 *
 *  \return  The current shared Steam Audio context, or nullptr if none is available.
 */
